...
console.log("direktorij je .....  "+ __dirname);

//require('node-jsx').install({
//    extension: '.jsx', harmony: true,
//    additionalTransform: function (src) {
//        return src.replace('require("client', 'require("' +  path.join(__dirname.replace('//','qq'),'./apps'));
//    }
//});
...
gulp.task('pages', function () {
    src.pages = [//*'apps/components/pages/**/*.jsx', 
        'apps/assets/pageindex.html',
        'apps/components/pages/404.html'
        ,'apps/*.jsx'
    ];

    //var currentPage = {};
    //var ActionTypes = require('./apps/constants/ActionTypes');
    //var apps = require('./apps/entry.js');
    ...
    var render = $.replace(/[\s\S]*/, fs.readFileSync('./apps/assets/appentry.html'))
      .on('error', function (err) { console.log(err); render.end(); });

    return gulp.src(src.pages)
      .pipe($.changed(DEST, { extension: '.html' }))
      .pipe($.if('*.jsx', render))// unnecesary for pageindex
        .pipe($.if('*.jsx', $.rename(function (path) {
            path.extname = ".html";
        })))
      ...
      .pipe($.if('**/pageindex.html', $.rename('index.html')))
      .pipe(gulp.dest(DEST))
